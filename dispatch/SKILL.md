...
description: >
  ...
  work units dispatched to independent Copilot CLI workers via tmux panes with
...
# Dispatch — Multi-Agent Orchestrator
...
| Pattern | Action |
|---------|--------|
| `dispatch [N workers on] REPO [with model MODEL] [: task descriptions]` | Launch new run |
| `dispatch resume [RUN_ID]` | Resume interrupted run |
| `dispatch status [RUN_ID]` | Show run status |
| `dispatch teardown [RUN_ID]` | Tear down workers and clean up |
...
CREATE TABLE IF NOT EXISTS dispatch_tasks (
    ...
    created_at TEXT DEFAULT (datetime('now')),
    updated_at TEXT DEFAULT (datetime('now')),
    FOREIGN KEY (run_id) REFERENCES dispatch_runs(run_id)
);
...
If `dispatch resume [RUN_ID]` → skip to STEP 9.
If `dispatch status` → query SQL + filesystem, report.
If `dispatch teardown [RUN_ID]` → run `~/bin/dispatch-workers.sh --teardown --run-id RUN_ID`.
...
base = os.path.expanduser(f"~/.copilot/dispatch/{run_id}")
...
```sql
INSERT INTO dispatch_runs (run_id, objective, repo_path, model, worker_count, total_tasks)
VALUES ('RUN_ID', 'OBJECTIVE', 'REPO', 'MODEL', N, T);

INSERT INTO dispatch_tasks (task_id, run_id, title, objective, branch)
VALUES ('task-001', 'RUN_ID', 'title', 'objective', 'dispatch/task-001');
...
```json
{
  ...
  "base": "~/.copilot/dispatch/run-20250715-143022",
  "objective": "...",
  "repo_path": "/abs/path",
  "model": "claude-sonnet-4-5",
  "phase": "dispatching|running|synthesizing|completed",
...
```bash
chmod +x ~/bin/dispatch-workers.sh
~/bin/dispatch-workers.sh \
  --run-id THE_RUN_ID \
  --count WORKER_COUNT \
  --repo THE_REPO_PATH \
  --model THE_MODEL \
...
```bash
tmux has-session -t "dispatch-THE_RUN_ID" 2>/dev/null && echo "FLEET_RUNNING" || echo "FLEET_FAILED"
```
...
SET completed_tasks = (SELECT COUNT(*) FROM dispatch_tasks WHERE run_id = 'RUN_ID' AND status = 'done'),
    failed_tasks = (SELECT COUNT(*) FROM dispatch_tasks WHERE run_id = 'RUN_ID' AND status = 'failed'),
...
print("=" * 60)
print("DISPATCH RESULTS SYNTHESIS")
print("=" * 60)
succeeded = failed = 0
...
for branch in $(git branch --list 'dispatch/*' | tr -d ' '); do
...
## STEP 9 — CRASH RECOVERY (dispatch resume)

```python
python3 -c '
import json, os, glob as g

dispatch_dir = os.path.expanduser("~/.copilot/dispatch")
run_id = "PROVIDED_OR_EMPTY"

if not run_id:
    runs = sorted(g.glob(f"{dispatch_dir}/run-*/state.json"), reverse=True)
    if not runs:
        print("No dispatch runs found.")
        exit(1)
    run_id = os.path.basename(os.path.dirname(runs[0]))

base = os.path.join(dispatch_dir, run_id)
...
Offer: `dispatch teardown RUN_ID`, `dispatch resume RUN_ID`, branch links.
...
| 17 | Branch name collision | `dispatch/{task_id}`, unique per task |
...
- [ ] `~/bin/dispatch-workers.sh` is executable
- [ ] `~/.copilot/agents/dispatch-worker.agent.md` is installed
- [ ] tmux is available
...
```bash
curl -s "https://raw.githubusercontent.com/DUBSOpenHub/skill-telemetry/main/docs/ping/dispatch.gif" > /dev/null 2>&1 &
```
